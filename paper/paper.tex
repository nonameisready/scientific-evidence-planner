...
surfaces a structural result: planner utility runs opposite to measured
community attention (pooled permutation $p = 0.004$), because both are
driven, with opposite sign, by the breadth of the existing evidence
base. We then report the robustness check that qualifies it: the effect
does not survive stratification by breadth and concentrates in exactly
the high-specificity stratum where Paper 3 documents its
retrieval-bounded label to under-count engagement, so the divergence
between resolution-driven and attention-driven allocation is real as a
mechanism but cannot yet be claimed as detected neglect. The framework
is released as a reference implementation with seven evaluation
metrics, full configurability, and no domain assumptions in code.
\end{abstract}
...
\paragraph{Whether that is neglect or measurement, this run cannot
say.} The tempting reading --- that the planner detects genuine
neglect --- does not survive the robustness checks that Paper 3's own
limitations demand, and we report the negative result rather than the
headline. Two decompositions (Table~\ref{tab:robust}) undercut it.
First, the pooled effect is largely \emph{breadth-mediated}: comparing
...
We therefore make only the weaker claim the data supports: planner
utility and measured community attention are driven with opposite sign
by the same evidence-structure feature, and the residual
anti-correlation is confounded with a documented measurement bound.
Separating genuine neglect from retrieval artifact requires
full-text-adjudicated outcomes on the high-specificity stratum --- a
concrete, bounded experiment ($n = 62$ here) rather than a caveat.
...
\begin{itemize}
  ...
  \item \textbf{The outcome label bounds what we can conclude.} Our
  central at-scale comparison is against a retrieval-bounded label that
  Paper 3 shows under-counts engagement non-uniformly across
  specificity. This is not a generic caveat but the specific reason the
  neglect interpretation is withheld in Section~\ref{sec:scale}. The
  decisive experiment is small and well-defined: full-text adjudication
  of the $62$ high-specificity tension questions, which would separate
  genuine neglect from measurement artifact directly.
\end{itemize}
...
community left behind --- but the robustness analysis says we have not
earned that reading. What the run establishes is narrower and still
worth having: planner utility and community attention are driven with
opposite sign by the same structural feature, and the part of that
divergence which might be genuine neglect is currently inseparable
from a documented measurement bound. Naming which of the two it is
requires full-text adjudication on 62 questions, not a larger corpus
--- and Paper 3's pre-registered prospective instance (frozen 2026,
scored 2027--2030) offers the second, time-graded test. The
infrastructure for both is now in place, and the more useful posture
is the one this paper ends in: a planner whose divergence from
community behaviour is measured rather than assumed.
