...
ranking toward the historically resolving actions. The framework is
released as a reference implementation with seven evaluation metrics,
full configurability, and no domain assumptions in code.
\end{abstract}
...
configuration; and
(6)~an open reference implementation with unit tests and a worked case
...
\begin{itemize}
  ...
  \item \textbf{Single worked case study.} The framework ships with one
  fully worked historical example (now aligned with the outcome Paper 2
  and Paper 3 document for it). The import path for Paper 3's $980$
  labeled questions exists and is exercised in the repository; the
  remaining work is constructing evidence graphs from its verbatim
  provenance clusters so each labeled question becomes a full benchmark
  unit, turning the seven metrics into distributions rather than a
  single run.
...
planning objective, imports the first as a calibrated configuration, and
on the series' own canonical case study recommends, first, the exact
action family that history used to settle the question. Whether that
agreement survives contact with hundreds of Paper 3's labeled questions
--- and with its pre-registered prospective instance --- is the next
